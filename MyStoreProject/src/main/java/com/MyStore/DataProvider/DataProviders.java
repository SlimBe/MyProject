...
//Class -->  EndToEndTest,  Test Case-->  endToEndTest	
	...
	// Class --> addcart orderpagetest, 
		@DataProvider(name = "addtocart")
		public Object[][] Addcart() {
			// Totals rows count
			int rows = obj.getRowCount("AddToCarte");
			// Total Columns
			int column = obj.getColumnCount("AddToCarte");
			int actRows = rows - 1;
			...
			for (int i = 0; i < actRows; i++) {
				for (int j = 0; j < column; j++) {
					data[i][j] = obj.getCellData("AddToCarte", j, i + 2);
				}
			}
			return data;
		}
